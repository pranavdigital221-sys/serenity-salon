...
export async function createRazorpayProductOrder(
  ...
  if (!isRazorpayConfigured()) {
    return {
      success: false,
      requiresConfiguration: true,
      error: 'Razorpay online payment is temporarily unavailable. You can complete payment upon delivery or arrival.',
    };
  }
  ...
  try {
    ...
    if (response.ok && responseData.id) {
      ...
      console.error(`[RAZORPAY PRODUCT ORDER CREATE FAILED]
HTTP ${httpStatus}
error.code = ${errorCode}
error.description = ${errorDescription}`);
