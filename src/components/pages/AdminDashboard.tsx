...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const handleDeleteAppointment = async (appointmentId: string) => {
    if (!window.confirm('Are you sure you want to permanently delete this appointment request?')) {
      return;
    }

    try {
      const success = await appointmentApi.delete(appointmentId);
      if (success) {
        setAppointments((prev) => prev.filter((a) => a.id !== appointmentId));
        if (selectedAppointmentForDetail?.id === appointmentId) {
          setSelectedAppointmentForDetail(null);
        }
        setActionFeedback('Appointment request removed from database.');
        setTimeout(() => setActionFeedback(null), 3000);
        const updatedStats = await appointmentApi.getStats();
        setStats(updatedStats);
      }
