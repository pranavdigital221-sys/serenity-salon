  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      if (!response.success || !response.order || !response.razorpayOrderId) {
        throw new Error(response.error || 'Online payment is temporarily unavailable. Please try again later.');
      }
      ...
      if (window && (window as any).Razorpay && razorpayOrderId && keyIdToUse && !keyIdToUse.includes('placeholder')) {
        ...
        const rzpOptions = {
          key: keyIdToUse,
          amount: Math.round((response.grandTotal || createdOrder.grandTotal || 0) * 100), // in paise
          currency: 'INR',
              ...
              {isTestMode && (
                <div className="p-3.5 rounded-xl bg-amber-50/90 border border-amber-300 text-amber-950 text-xs mb-3 space-y-1.5">
                  ...
                  <p className="text-[11px] text-amber-800 leading-relaxed">
                    This store is connected to Razorpay Sandbox (Test Mode). No real money will be charged. Do not enter real personal banking credentials or real bank OTPs.
                  </p>
                  <div className="text-[10px] text-amber-950 font-mono bg-amber-100/70 p-2 rounded-lg space-y-1">
                    ...
                    <div>• <span className="font-semibold">Netbanking:</span> Razorpay presents a simulated test bank page.</div>
