  ...
  const handleConfirmAndPay = async () => {
    ...
    try {
      ...
      if (!orderRes.success || !orderRes.orderId) {
        throw new Error(orderRes.error || 'Failed to initiate secure payment order with gateway.');
      }
            ...
            {isTestMode && (
              <div className="p-3.5 rounded-xl bg-amber-50/90 border border-amber-300 text-amber-950 text-xs mb-4 space-y-1.5">
                ...
                <p className="text-[11px] text-amber-800 leading-relaxed">
                  This checkout is running in Razorpay Sandbox (Test Mode). Real money will not be deducted. Do not enter real personal banking credentials or real bank OTPs.
                </p>
                <div className="text-[10px] text-amber-950 font-mono bg-amber-100/70 p-2 rounded-lg space-y-1">
                  <div>• <span className="font-semibold">Test UPI:</span> Enter VPA <span className="font-bold underline">success@razorpay</span> (or <span className="underline">failure@razorpay</span> to test failure).</div>
                  <div>• <span className="font-semibold">Netbanking:</span> Razorpay presents a simulated test bank page. Any simulated OTP will succeed.</div>
                  <div>• <span className="font-semibold">Cards:</span> Standard Razorpay test card credentials are accepted.</div>
                </div>
              </div>
            )}
